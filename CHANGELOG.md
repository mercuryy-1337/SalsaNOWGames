...
### Added
- **Download Tip Box** - Helpful note on the download page explaining that downloads may appear stuck
  - Informs users that percentage updates in chunks, not in real-time
  - Displayed next to the "Logged in as" status box

### Changed
- **Improved Download Logging** - Better visibility into download activity
  - Download start is now logged with game name and AppID
  - Download failures are logged with the error reason
  - Removed verbose header image download logs
